...
def main():
	...
	q_learning_game(Huarongdao(zhen), 500, '逃之夭夭')
	print('finished 逃之夭夭')
	# Zhen form:
	zhen = WHZJ()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '无横之局')
	print('finished 无横之局')
	'''
	# Zhen form:
	zhen = JDHL()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '将当后路')
	print('finished 将当后路')
	# Zhen form:
	zhen = QHHY()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '前呼后拥')
	print('finished 前呼后拥')
	# Zhen form:
	zhen = BYHK()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '比翼横空')
	print('finished 比翼横空')
	# Zhen form:
	zhen = QGWG()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '巧过五关')
	print('finished 巧过五关')
	'''
	# Zhen form:
	zhen = WJBG()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '五将逼宫')
	print('finished 五将逼宫')
	# Zhen form:
	zhen = BLCY()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '兵临曹营')
	print('finished 兵临曹营')
	# Zhen form:
	zhen = SJLG()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '四将连关')
	print('finished 四将连关')
	# Zhen form:
	zhen = XJZZC()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '新近在咫尺')
	print('finished 新近在咫尺')
	# Zhen form:
	zhen = XLQB()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '星罗棋布')
	print('finished 星罗棋布')
	# Zhen form:
	zhen = SMBF()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '四面八方')
	print('finished 四面八方')
	# Zhen form:
	zhen = NQCT()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '牛气冲天')
	print('finished 牛气冲天')
	# Zhen form:
	zhen = DBQJ()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '调兵谴将')
	print('finished 调兵谴将')
	# Zhen form:
	zhen = BSZL()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '背水列阵')
	print('finished 背水列阵')
	# Zhen form:
	zhen = HDLM2()	# from games.py
	q_learning_game(Huarongdao(zhen), 500, '横刀立马2')
